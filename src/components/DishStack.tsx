...
export default function DishStack() {
  ...
  // Track if this section is in the viewport
  useEffect(() => {
  ...
  // Wheel navigation only when in view
  useEffect(() => {
    if (!isInView) return;

    const handleWheel = (e: WheelEvent) => {
      // Only capture if we're not at the edges
      const atStart = currentIndex === 0 && e.deltaY < 0;
      const atEnd = currentIndex === items.length - 1 && e.deltaY > 0;
  ...
  }, [isInView, navigate, currentIndex, items.length]);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-[2] h-full flex flex-col lg:flex-row">
        {/* Left: Dish info */}
        <div className="flex flex-col justify-center items-start px-8 sm:px-12 lg:px-20 lg:w-[45%] h-[35vh] lg:h-full">
          ...
          <h2
            className="font-display italic text-2xl sm:text-3xl mb-6 season-transition"
          ...
          {/* Pairings */}
          <p
            className="font-body text-[12px] mt-6"
            style={{ color: "#B8B0A3" }}
          >
