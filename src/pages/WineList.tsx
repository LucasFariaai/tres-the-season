...
const baseWines = (wineList as WineItem[]).filter((wine) =>
  ["sparkling", "white", "red"].includes(wine.type),
) as WineItem[];
...
const FEATURED_GRAPES = ["Chardonnay", "Pinot Noir", "Nebbiolo", "Riesling"];
...
function toggleValue<T extends string | number>(items: T[], value: T) {
...
function buildGrapeOptions(wines: WineItem[]) {
  ...
  const dynamic = [...counts.entries()]
    .filter(([grape, count]) => count >= 3 && !FEATURED_GRAPES.includes(grape))
    .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
    .map(([grape]) => grape);

  return [...FEATURED_GRAPES, ...dynamic];
}

function filterWines(wines: WineItem[], filters: ActiveFilters) {
  ...
  return wines.filter((wine) => {
    ...
    if (filters.regions.length > 0 && !filters.regions.includes(getGroupKey(wine))) return false;
    if (
    ...
    if (query && !getSearchIndex(wine).includes(query)) return false;
    return true;
  });
}

function buildSections(wines: WineItem[], activeTypes: WineType[]) {
  const orderedTypes = (activeTypes.length > 0 ? TYPE_OPTIONS.map((option) => option.value).filter((type) => activeTypes.includes(type)) : TYPE_OPTIONS.map((option) => option.value));

  return orderedTypes
    .map((type) => {
      const typeWines = wines.filter((wine) => wine.type === type);
      const groupsMap = new Map<string, RegionGroup>();

      for (const wine of typeWines) {
        const key = getGroupKey(wine);
...
function WineToggle({ active, children, onClick }: { active: boolean; children: React.ReactNode; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn(
        ...
        active
          ...
          : "border-[rgba(248,239,230,0.18)] bg-transparent text-wine-text hover:border-[rgba(248,239,230,0.32)] hover:text-wine-muted",
      )}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "border-transparent bg-wine-accent text-wine-text"
          : "border-[rgba(248,239,230,0.12)] bg-transparent text-wine-muted hover:border-[rgba(248,239,230,0.24)] hover:text-wine-text",
      )}
      style={{ borderRadius: "999px" }}
    >
      {label}
...
function WineRow({
  ...
  return (
    <article
      className={cn(
        "grid items-start gap-4 border-t border-[rgba(248,239,230,0.10)] py-5 sm:grid-cols-[36px_minmax(0,1fr)_92px_70px] sm:gap-5",
        className,
      )}
    >
      <div className={cn("wine-list-body text-[12px] uppercase tracking-[0.12em] text-wine-micro", !showOrdinal && "hidden sm:block") }>
      ...
      <div className="wine-list-body text-[11px] uppercase tracking-[0.12em] text-wine-micro sm:pt-1 sm:text-right">
        ...
        <p className="mt-1">{getRegionLabel(wine)}</p>
      </div>
...
export default function WineListPage() {
  ...
  const featuredWines = useMemo(
    () => FEATURED_IDS.map((id) => baseWines.find((wine) => wine.id === id)).filter(Boolean) as WineItem[],
    [],
  );

  const grapeOptions = useMemo(() => buildGrapeOptions(baseWines), []);

  const regionOptions = useMemo(() => {
    if (filters.countries.length === 0) return [];
    const typedWines = filters.types.length > 0 ? baseWines.filter((wine) => filters.types.includes(wine.type as WineType)) : baseWines;
    const countries = typedWines.filter((wine) => filters.countries.includes(wine.country));
    return [...new Set(countries.map((wine) => getGroupKey(wine)))].sort((a, b) => a.localeCompare(b));
  }, [filters.countries, filters.types]);

  const filteredWines = useMemo(() => filterWines(baseWines, filters), [filters]);
  ...
  }, [isOpen, sections.length, filteredWines.length, filters.query, filters.countries, filters.grapes, filters.regions, filters.types]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => setIsSticky(isOpen && !entry.isIntersecting),
      { threshold: 1 },
    );
  ...
  useGSAP(
    () => {
      if (!isOpen || reducedMotion) {
        gsap.set("[data-animate='heading'], [data-animate='region'], [data-animate='row']", { clearProps: "all" });
        return;
      }
      ...
      rows.forEach((row, index) => {
        timeline.to(row, { opacity: 1, duration: 0.4 }, `>-0.1+${Math.floor(index / 10) * 0.04}`);
      });
    },
    { dependencies: [isOpen, reducedMotion, sections], scope: animationScope },
  );
  ...
  return (
    <div className="wine-list-theme min-h-screen">
      <SeasonBar />
      ...
      <main className="relative overflow-x-clip bg-wine-bg text-wine-text">
        <section className="px-5 pb-20 pt-[120px] sm:px-8 lg:px-12">
          <div className="mx-auto max-w-[1400px]">
            ...
            <h1 className="wine-list-display mt-5 max-w-5xl text-[clamp(48px,7vw,96px)] text-wine-text">
              A collection of gems.
            </h1>
            <p className="wine-list-display mt-6 max-w-3xl text-[clamp(18px,2.5vw,28px)] text-wine-muted">
          ...
          <div className="mx-auto max-w-[1400px]">
            <div className="mb-10 flex items-end justify-between gap-6">
              <div>
                ...
                <h2 className="wine-list-display mt-4 text-[clamp(32px,4.5vw,56px)] text-wine-text">Conversation starters.</h2>
              </div>
            </div>
            ...
            <div className="mt-10 text-center">
              <button
                ...
                className="wine-list-display text-[18px] text-wine-muted transition-colors duration-300 hover:underline hover:text-wine-text"
              >
                See the full list
              </button>
            </div>
        ...
        <section ref={fullListRef} className="px-5 py-20 sm:px-8 lg:px-12">
          ...
          <div
            className={cn(
              "z-30 border-y border-[rgba(248,239,230,0.10)] bg-wine-bg transition-[top,left,right,width] duration-300",
              isSticky ? "fixed left-0 right-0 top-0" : "sticky top-0",
            )}
          >
            <div className="mx-auto flex max-w-[1400px] flex-col gap-4 px-5 py-4 sm:px-8 lg:px-12 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex items-start justify-between gap-4 lg:pt-1">
                ...
                <button
                  ...
                  className="ml-auto flex h-11 w-11 items-center justify-center border border-transparent text-wine-micro transition-colors duration-300 hover:text-wine-text"
                  aria-label="Collapse full wine list"
                >
          ...
          <div
            ...
            style={{ height: 0, transition: reducedMotion ? "opacity 0.01s linear" : `height 600ms ${EASE}, opacity 300ms linear`, opacity: isOpen ? 1 : 0 }}
          >
            <div ref={animationScope} className="mx-auto max-w-[1400px] pt-10">
              {isOpen ? (
                sections.length > 0 ? (
                    ...
                    {sections.map((section) => (
                      <section key={section.type} className="space-y-8">
                        <div data-animate="heading">
                          ...
                          <h2 className="wine-list-display mt-3 text-[clamp(36px,5vw,64px)] text-wine-text">{section.label}</h2>
                        </div>
                          ...
                          {section.groups.map((group) => (
                            <div key={group.key} className="space-y-4">
                              <div data-animate="region" className="flex items-center gap-4 pt-3">
                                ...
                                <p className="wine-list-body shrink-0 text-[12px] uppercase tracking-[0.15em] text-wine-micro">
                                  {group.label}
                                </p>
                              </div>
                              ...
                              <div className="transition-all duration-300">
                                {group.wines.map((wine, index) => (
                                  <WineRow
                                    ...
                                    index={index}
                                    showOrdinal={false}
                                    className="data-[animate=row]:transition-all data-[animate=row]:duration-300"
                                  />
                                ))}
                              </div>
                            </div>
